...
class ModerationFormTest extends ModerationStateTestBase {
   ...
   *
   * List of transitions should list the current state as an option.
   */
  public function testSameStateTransition() {
    // Create new moderated content in draft.
    $this->drupalPostForm('node/add/moderated_content', [
      'title[0][value]' => 'Some moderated content',
      'body[0][value]' => 'First version of the content.',
    ...
    $select = $this->getSession()->getPage()->findField('new_state');
    $this->assertNotEmpty($select, 'Could not find select element');

    $option = $select->find('named', ['option', 'draft']);
    $this->assertNotEmpty($option, 'Could not transition from current state to current state');
  }
