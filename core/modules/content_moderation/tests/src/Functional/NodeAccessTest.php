...
class NodeAccessTest extends ModerationStateTestBase {
  ...
  /**
   * Test per-bundle unpublished content permission.
   */
  public function testPerBundlePermissions() {
    // Disable private nodes as this test is about per-bundle unpublished perms.
    \Drupal::state()->set('node_access_test.private', TRUE);
    ...
    // Verify per-bundle unpublished permissions are working.
    $user = $this->createUser([
      'use editorial transition create_new_draft',
      'view latest version',
      'view any unpublished node:moderated_content content',
    ]);
    $this->drupalLogin($user);
